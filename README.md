<h1 align="center">
  <br>
  <a href=""><img src="https://github.com/srhady/Hady/raw/refs/heads/main/bingsport-tth-logo.webp" alt="Bingstream" width="120"></a>
  <br>
  Bingstream Live Sports Playlist
  <br>
</h1>

<h2 align="center">A Script to trigger GitHub Actions to automatically update VIP Live Streaming Links</h2>

</a>
  <a href="">
      <img src="https://github.com/srhady/Hady/raw/refs/heads/main/IMG_20260310_134701.jpg">
  </a>
​<p align="center">
<img src="https://img.shields.io/badge/Made_With-Node.js-green?style=for-the-badge&logo=nodedotjs" alt="NodeJS">
<img src="https://img.shields.io/badge/Made%20in-Bangladesh_🇧🇩-green?colorA=%23ff0000&colorB=%23017e40&style=for-the-badge" alt="Bangladesh">
<img src="https://img.shields.io/badge/last--commit--time-03:17%20PM,%2019%20May%202026-blue?style=for-the-badge" alt="Last Commit">
</p>



# 📒 Introduction

**⚡ Bingstream Auto-Playlist** is a lightweight and smart script built with Node.js and Puppeteer that extracts live sports stream links directly from Bingstream.info and converts them into an auto-updating **M3U playlist**.

⏱️ The script runs automatically via **GitHub Actions**, dynamically bypassing token locks and frame protections to fetch fresh, working VIP `.m3u8` streaming links — ready to play instantly!

📡 Catch all the major live events, including **Football, Cricket, Basketball, Tennis**, and more — directly through any M3U-supported player [ NS Player, Televizo ]

> ⚠️ For educational and personal use only. Please follow the laws and regulations of your country.

# 💥 Key Features

* 🔄 Completely Automated M3U playlist generation.
* ⚡ Smart extraction that bypasses dynamic token protections.
* 🛡️ Obfuscated core script to prevent abuse and ensure long-term stability.
* 📺 Premium VIP links are automatically fetched and formatted.
* 🚀 Plug & Play ready for any standard IPTV player.

# Free Sports Live
<div align="center">
  <a href="https://srhady.github.io/Hady/">
    <img src="https://github.com/srhady/Hady/raw/refs/heads/main/.photo/IMG_20260316_011540.jpg" alt="Hady's Website" width="100%">
  </a>
  <br>
  <br>
  <b>🌐 <a href="https://livesportsplay.vercel.app/">Visit My Website</a></b>
</div>

<br>





# 🕹️ How To Use
**Direct Use**
* 👉 **[Auto Updated M3U Playlist File](https://raw.githubusercontent.com/srhady/bingstream/main/playlist.m3u)**
* Add this raw link directly to your player.
**For Developer**
* 👉 **[Auto Updated JSON File](https://raw.githubusercontent.com/srhady/bingstream/main/playlist.json)**


# 🎬 How To Play
**📱 Android / iOS**
* Use Network Stream Player.
* Add This PlayList URL: `https://raw.githubusercontent.com/srhady/bingstream/main/playlist.m3u`
* Enjoy 😊

**🖥️ Android TV / Smart TV**
* Use Televizo.
* Add This PlayList URL: `https://raw.githubusercontent.com/srhady/bingstream/main/playlist.m3u`
* Enjoy 😊

<p align="center">
<a href="https://raw.githubusercontent.com/srhady/Hady/main/playlist.m3u">
<img src="https://github.com/srhady/Hady/raw/main/.photo/Screenshot_20260310_162538.jpg" width="45%"> <img src="https://github.com/srhady/Hady/raw/main/.photo/Screenshot_20260310_162712.jpg" width="45%">
<br><br>
<img src="https://github.com/srhady/Hady/raw/main/.photo/Screenshot_20260310_162014.jpg" width="92%">
<br><br>
<img src="https://github.com/srhady/Hady/raw/main/.photo/Screenshot_20260310_162717.jpg" width="45%">
</a>
</p>




# 🚬 Author 
[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&pause=100&color=FF2C10&background=31FF9400&width=400&lines=DEVELOPED+BY+HADY)](https://git.io/typing-svg)

# 📝 Note
* The following code is for educational purposes only. It demonstrates how to automate web scraping and format IPTV playlists. Do not use it for any illegal or harmful activities.
* Please give proper credit if you share this content. Otherwise, I will take it down.
* The core source code of this repository is strictly obfuscated to ensure security and prevent unauthorized modifications. 
* Works globally for all sports lovers

# ✉️ Find Me on 

- [![Github](https://img.shields.io/badge/Github-SRHADY-purple?style=for-the-badge&logo=github)](https://github.com/srhady)

- [![Telegram](https://img.shields.io/badge/Telegram-LiveSportsPlay-indigo?style=for-the-badge&logo=telegram)](https://t.me/livesportsplay)
